...
def load_spacy_model():
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: spaCy model not available. Knowledge graph will be limited. Error: {e}")
            return None
...
def hybrid_search(
    ...
    try:
        ...
        return rag_pipeline.retrieve(query, top_k, chunks)
...
def create_knowledge_graph(chunks: List[Dict]) -> str:
    ...
    try:
        if nlp is None:
            return "<html><body><h3>Knowledge graph requires spaCy model. Please install: python -m spacy download en_core_web_sm</h3></body></html>"
        ...
        return f"<html><body><h3>Error generating knowledge graph: {str(e)}</h3></body></html>"
...
def cluster_chunks(rag_pipeline, num_clusters: int = 5):
    ...
    try:
        ...
        fig, ax = plt.subplots(figsize=(12, 8))
        ax.text(0.5, 0.5, f'Error during clustering: {str(e)}', 
